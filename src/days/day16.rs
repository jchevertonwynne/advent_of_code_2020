...
fn generate_keys<'a>(rows: &'a [Row], tickets: &'_ [&Ticket]) -> Vec<&'a str> {
    ...
    while !fixing_possibilities.is_empty() {
        let singular = fixing_possibilities
            .drain_filter(|_, v| v.len() == 1)
            .collect::<Vec<_>>();
        for (label, singular_index) in singular {
            let index = singular_index.iter().next().expect("should have one value");
    ...
    (0..20).map(|i| *fixed.get(&i).expect("exist")).collect()
}
...
fn part2(rows: &[Row], valid: &[&Ticket], ticket: Ticket) -> usize {
    ...
    for (&key, &val) in keys.iter().zip(ticket.iter()) {
